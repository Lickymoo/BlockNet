...
public class ProvisionCommandMaster extends CommandExecutor{
	...
	public CommandResponse exec(String[] args) {
		...
		ProvisionedServerModel provisioned = blockNet.provisionServer(args[1]);
		if(provisioned == null) {
			log("All slaves were at maximum capacity");
			log("Consider modifying server weights or adding more servers to network");
			return CommandResponse.OK;
		}
		log("Sucessfully provisioned template \"" + args[1] + "\" instance on" + provisioned.getServlet().getIp() + ":" + provisioned.getPort() + ":" + provisioned.getUuid().toString());
		return CommandResponse.OK;
	}
}
